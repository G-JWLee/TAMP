...
class Videollama2MetaForCausalLM(ABC):
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        if audio_tower is None:
            ...
            temp_audio_masks = []
            temp_video_masks = []
            for i in range(len(mm_features)):
                temp_audio_masks.append(torch.zeros(len(mm_features[i]), dtype=torch.bool))
                temp_video_masks.append(torch.ones(len(mm_features[i]), dtype=torch.bool))
            ...
            for audio_idx, videoimage_idx in zip(select_audio_id, select_videoimage_id):
                if audio_idx and videoimage_idx:
                    ...
                    a_mask = torch.zeros(len(mm_features[-1]), dtype=torch.bool)
                    v_mask = torch.zeros(len(mm_features[-1]), dtype=torch.bool)
                    a_mask[-len(Xa_features[idx_a]):] = True
                    v_mask[:len(X_features[idx_v])] = True
                    temp_audio_masks.append(a_mask)
                    temp_video_masks.append(v_mask)
                    idx_a += 1
                    idx_v += 1
                elif audio_idx:
                    mm_features.append(Xa_features[idx_a])
                    a_mask = torch.ones(len(mm_features[-1]), dtype=torch.bool)
                    v_mask = torch.zeros(len(mm_features[-1]), dtype=torch.bool)
                    temp_audio_masks.append(a_mask)
                    temp_video_masks.append(v_mask)
                    idx_a += 1
                    ...
                    a_mask = torch.zeros(len(mm_features[-1]), dtype=torch.bool)
                    v_mask = torch.ones(len(mm_features[-1]), dtype=torch.bool)
                    temp_audio_masks.append(a_mask)
                    temp_video_masks.append(v_mask)
                    idx_v += 1
                else:
            ...
            temp_video_masks = []
            for i in range(len(mm_features)):
                temp_audio_masks.append(torch.ones(len(mm_features[i]), dtype=torch.bool))
                temp_video_masks.append(torch.zeros(len(mm_features[i]), dtype=torch.bool))
        ...
        for batch_idx, cur_input_ids in enumerate(input_ids):
            ...
            cur_video_token_masks = []
            cur_audio_token_masks = []
            ...
            while mm_token_indices.numel() > 0:
                ...
                cur_video_token_masks.append(torch.zeros(len(cur_new_input_embeds[-1]), dtype=torch.bool))
                cur_audio_token_masks.append(torch.zeros(len(cur_new_input_embeds[-1]), dtype=torch.bool))

                cur_new_input_embeds.append(cur_mm_features)
                cur_video_token_masks.append(torch.logical_and(torch.ones(len(cur_new_input_embeds[-1]), dtype=torch.bool), temp_video_masks[cur_mm_idx]))
                cur_audio_token_masks.append(torch.logical_and(torch.ones(len(cur_new_input_embeds[-1]), dtype=torch.bool), temp_audio_masks[cur_mm_idx]))
            ...
            cur_video_token_masks = [x.to(device=self.device) for x in cur_video_token_masks]
            cur_audio_token_masks = [x.to(device=self.device) for x in cur_audio_token_masks]

            # NOTE: one cur_new_input_embeds per each  
            cur_new_input_embeds = torch.cat(cur_new_input_embeds, dim=0)
            cur_video_token_masks = torch.cat(cur_video_token_masks, dim=0)
            cur_audio_token_masks = torch.cat(cur_audio_token_masks, dim=0)

            new_input_embeds.append(cur_new_input_embeds)
            video_token_masks.append(cur_video_token_masks)
            audio_token_masks.append(cur_audio_token_masks)
        ...
        if any(x.shape != new_input_embeds[0].shape for x in new_input_embeds):
            ...
            video_token_masks_align = []
            for cur_video_token_mask in video_token_masks:
                cur_video_token_mask = torch.cat((cur_video_token_mask, torch.zeros((max_len - cur_video_token_mask.shape[0]), dtype=cur_video_token_mask.dtype, device=cur_video_token_mask.device)), dim=0)
                video_token_masks_align.append(cur_video_token_mask)
            video_token_masks = torch.stack(video_token_masks_align, dim=0)

            audio_token_masks_align = []
            for cur_audio_token_mask in audio_token_masks:
                cur_audio_token_mask = torch.cat((cur_audio_token_mask, torch.zeros((max_len - cur_audio_token_mask.shape[0]), dtype=cur_audio_token_mask.dtype, device=cur_audio_token_mask.device)), dim=0)
                audio_token_masks_align.append(cur_audio_token_mask)
            audio_token_masks = torch.stack(audio_token_masks_align, dim=0)
            ...
            video_token_masks = torch.stack(video_token_masks, dim=0)
            audio_token_masks = torch.stack(audio_token_masks, dim=0)
            if labels is not None:
        ...
        self.video_token_masks = video_token_masks
        self.audio_token_masks = audio_token_masks
