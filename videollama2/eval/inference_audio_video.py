...
from videollama2.pruners.structural_pruner import prune_blocks
...
class AVQADataset(Dataset):
    ...
    def __init__(self, questions, processor, video_folder):
    ...
    def __getitem__(self, idx):
        ...
        video_path  = os.path.join(self.video_folder, sample['video'])
...
def run_inference(args):
    ...
    if args.block_pruning_config is not None:
        prune_blocks(model, args.block_pruning_config)
    ...
    if args.dataset == "AVQA":
        dataset = AVQADataset(gt_questions, processor['video'], args.video_folder)
    elif args.dataset == "AVSD":
        dataset = AVSDDataset(gt_questions, processor['video'], args.video_folder)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--max_new_tokens", type=int, required=False, default=64)

    parser.add_argument("--block_pruning_config", type=str, default=None)
