...
from videollama2.pruners.base_pruner import BasePruner


def cos_pairwise_density(embeddings, video_mask, audio_mask):
    ...
    a_embeddings = a_embeddings / a_embeddings.norm(dim=1, keepdim=True)
    ...
    return v_mean_dist, a_mean_dist, l_mean_dist, vl_mean_dist, al_mean_dist, av_mean_dist
...
class ActivationDensity:
    ...
    def add_batch(self, inp, out, video_mask=None, audio_mask=None, score=None):
        assert len(out) == 1
        v_density, a_density, l_density, vl_dist, al_dist, av_dist = cos_pairwise_density(out[0], video_mask=video_mask[0], audio_mask=audio_mask[0])

        self.v_density *= self.nsamples / (self.nsamples+1)
        self.a_density *= self.nsamples / (self.nsamples+1)
        self.l_density *= self.nsamples / (self.nsamples+1)
        self.vl_dist *= self.nsamples / (self.nsamples+1)
        self.al_dist *= self.nsamples / (self.nsamples+1)
        self.av_dist *= self.nsamples / (self.nsamples+1)

        self.nsamples += 1

        self.v_density += v_density / self.nsamples
        self.a_density += a_density / self.nsamples
        self.l_density += l_density / self.nsamples
        self.vl_dist += vl_dist / self.nsamples
        self.al_dist += al_dist / self.nsamples
        self.av_dist += av_dist / self.nsamples


class ActivationTotalDensity:
    """
    This class wraps a GPT layer for specific operations.
    """
    ...
    def add_batch(self, inp, out, video_mask=None, audio_mask=None, score=None):
        assert len(out) == 1
        density = cos_pairwise_total_density(out[0])

        self.density *= self.nsamples / (self.nsamples+1)
        self.nsamples += 1

        self.density += density / self.nsamples
...
class LayerSparsity:
    ...
    def compute_importance_scores(self, layer_to_group_mapping):
        ...
        for d in data_loader:
            d['images'][0][0]['video'] = d['images'][0][0]['video'].to(dtype=torch.float16, device='cuda', non_blocking=True)
            d['images'][0][0]['audio'] = d['images'][0][0]['audio'].to(dtype=torch.float16, device='cuda', non_blocking=True)
    ...
    def compute_density(self, layer_to_group_mapping):
        ...
        with torch.no_grad():
            inps, outs, caches, video_masks, audio_masks = prepare_calibration_input_encoder(model, data_loader, 'model', 128, 'model.layers')
        ...
        for i in range(len(layers)):
            ...
            def add_batch(name):
                def tmp(_, inp, out):
                    wrapped_layers[name].add_batch(inp[0].data, out.data, video_masks[j], audio_masks[j])
                return tmp
            ...
            for name in subset:
                v_density, a_density, l_density = wrapped_layers[name].v_density, wrapped_layers[name].a_density, wrapped_layers[name].l_density
                vl_dist, al_dist, av_dist = wrapped_layers[name].vl_dist, wrapped_layers[name].al_dist, wrapped_layers[name].av_dist
                full_name = f'model.layers.{i}.{name}.weight'
                density_dict[full_name] += sum([1 - v_density, 1 - a_density, 1 - l_density, 1 - vl_dist, 1 - al_dist, 1 - av_dist])
