...
class LLaVALayerMagnitudePruner(LayerWiseBasePruner):
    pruner_name = "llava_magnitude_pruner"
    def __init__(
        self,
        model,
        data_loader,
    ...
    def global_iterative_pruning(self, target_sparsity, dict_layers_to_prune, iteratation=1, max_sparsity_per_layer=1.0):
        ...
        for i in range(1, iteratation+1):
            ...
            importance_measure = self.compute_importance_scores(
                self.model, self.data_loader, dict_layers_to_prune, None
            )
        ...
        for k, v in self.model.named_parameters():
            sparsity = (v == 0).float().sum() / v.numel()
            print(f"Layer: {k}, Sparsity: {sparsity:.4f}")
    ...
    def prune(self, importance_scores=None, keep_indices_or_masks=None):
        ...
        # vit_keep_ratio = min(t5_keep_ratio, vit_keep_ratio) # this code only works when vit_keep_ratio == t5_keep_ratio
        ...
        self.model = self.global_iterative_pruning(
            target_sparsity= 1 - sparsity_ratio,  
