...
from llava.pruners.layer_single_base_pruner import LayerWiseBasePruner, LayerSparsity
...
class LLaMALayerWandaPruner(LayerWiseBasePruner):
    ...
    def _prune(self, model, dataloader, model_prefix, module_to_process="encoder.block", n_samples=128, sparsity_ratio=0.5, token_selection='naive'):
        ...
        with torch.no_grad():
            inps, outs, caches, image_masks = self.prepare_calibration_input_encoder(model, dataloader, model_prefix, n_samples, module_to_process)
        ...
        for i in range(len(layers)):
                ...
                for j in range(n_samples):
                    with torch.no_grad():
                        ...
                        if "llama" in model.config.model_type:
                            ...
                            q_state, k_state = apply_rotary_pos_emb_qwen(q_state, k_state, cos, sin, caches[j]['position_ids'])                 
                            ...
                            raise ValueError(f"Attention not defined for {model.config.model_type}")
            ...
            def add_batch(name):
                def tmp(_, inp, out):
                    wrapped_layers[name].add_batch(inp[0].data, out.data, image_masks[j], scores[j])
                return tmp
...
class VITLayerWandaPruner(LayerWiseBasePruner):
    ...
    @print_time
    def _prune(self, model, dataloader, model_prefix, module_to_process="encoder.block", n_samples=128, sparsity_ratio=0.5):
        with torch.no_grad():
...
class LLaVALayerWandaPruner(LayerWiseBasePruner):
    pruner_name = "llava_wanda_pruner"
    def __init__(
        self,
        model,
        data_loader,
