...
from llava.analysis.activation_analysis.activation_density import cos_pairwise_density
...
class LLaMALayerSparseGPTPruner(LayerWiseBasePruner):
    ...
    def _prune(self, model, dataloader, model_prefix, module_to_process="encoder.block", n_samples=128, sparsity_ratio=0.5, token_selection='naive'):
        ...
        for i in tqdm(range(len(layers)), total=len(layers)):
            layer = layers[i]
            subset = find_layers(layer)
            ...
            for name in subset:
                if torch.isinf(wrapped_layers[name].layer.weight).any():
                    wrapped_layers[name].layer.weight.data[torch.isinf(wrapped_layers[name].layer.weight.data)] = 0
...
class LLaVALayerSparseGPTPruner(LayerWiseBasePruner):
