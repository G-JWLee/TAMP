...
class LazySupervisedDataset(Dataset):
    def __init__(self, data_path: str, tokenizer: transformers.PreTrainedTokenizer, image_processor, data_args):
        ...
        if data_args.sample_select == 'random':
            sampled_indices = np.random.choice(np.arange(task_start_id, task_end_id), size=data_args.nsamples, replace=False)
...
def create_data_loader(tokenizer, image_processor, data_args, num_workers=4):
    ...
    dataset = LazySupervisedDataset(data_path=data_args.data_path, tokenizer=tokenizer, 
                                    image_processor=image_processor, data_args=data_args)
